...
truncator_fn <- function(x) {
  ...
  if (!('min' %in% names(inputs))) {
    ...
    x[x <= inputs$min] <- syberiaMungebits:::trunc.dig(inputs$min,
      digits = syberiaMungebits:::MAX_DISCRETIZATION_DIGITS - 1)
    x[x >= inputs$max] <- syberiaMungebits:::trunc.dig(inputs$max,
      digits = syberiaMungebits:::MAX_DISCRETIZATION_DIGITS - 1)
  }
  x
}
